...
import kotlinx.android.synthetic.main.audio_list_layout.*
...
class AudioList : AppDrawer(), AudioListAdapter.OnClickChange, View.OnClickListener,
    RetrofitResponse {
    private var audioListAdapter: AudioListAdapter? = null
    private var audioList = ArrayList<ViewSavedMultimedia.ChatMultiMedia>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvMainTitle.text = getString(R.string.Audio)
        selectedMedia.clear()
        getIntentData()
        setClickCallbacks()
    }
    ...
    private fun getIntentData() {
        try {
            if (intent.hasExtra("audioList")) {
                var auList =
                    intent.getSerializableExtra("audioList") as ArrayList<ViewSavedMultimedia.ChatMultiMedia>
                audioList.addAll(auList)

                if (audioList.size==0)
                {
                    showToast("Audios not shared yet")
                }
            }
            setAudioListAdapter()
        } catch (e: Exception) {
                            ...
                            for (i in 0 until selectedMedia.size) {
                                if (names.contains(selectedMedia[i])) {
                                    ...
                                    if (i == selectedMedia.size - 1) {
                                        DownloadTask(
                                            this,
                                            WebUrls.CHAT_MEDIA_URL + audioList[indexIs].message,
                                            true
                                        )
                                    } else {
                                        DownloadTask(
    ...
    private fun setAudioListAdapter() {
        try {
            ...
            rvAudio.adapter = audioListAdapter
            audioListAdapter!!.initOnClickChange(this)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    override fun checkChange(position: Int, isChecked: Boolean) {
        try {
            audioList[position].isSelected = isChecked
            if (isChecked) {
                if (!selectedMedia.contains(audioList[position].id)) {
                    selectedMedia.add(audioList[position].id)
                }
            } else {
                if (selectedMedia.contains(audioList[position].id)) {
                    var indexIs = selectedMedia.indexOf(audioList[position].id)
        ...
        try {
            startActivity(
                Intent(this, FullMediaScreen::class.java)
                    .putExtra("file_type", audioList[position].message_type)
                    .putExtra("message_type", "media")
                    .putExtra("message", audioList[position].message)
            )
        } catch (e: Exception) {
            e.printStackTrace()
        }
                ...
                try {
                    ...
                    if (jsonObject.getBoolean("response")) {
                        ...
                        for (i in 0 until selectedMedia.size) {
                            if (names.contains(selectedMedia[i])) {
                                var indexIs = names.indexOf(selectedMedia[i])
                                audioList.removeAt(indexIs)
                                audioListAdapter!!.notifyItemRemoved(indexIs)
                                audioListAdapter!!.notifyItemRangeChanged(indexIs, audioList.size)
                            }
                        }
                        selectedMedia.clear()
                    }
