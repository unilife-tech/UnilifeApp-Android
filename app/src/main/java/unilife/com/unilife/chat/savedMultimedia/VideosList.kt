...
import kotlinx.android.synthetic.main.videos_list_layout.*
...
class VideosList : AppDrawer(), VideoListAdapter.OnClickChange, View.OnClickListener,
    RetrofitResponse {
    private var videoList = ArrayList<ViewSavedMultimedia.ChatMultiMedia>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.videos_list_layout)
        ...
        tvMainTitle.text = getString(R.string.Video)
        selectedMedia.clear()
        getIntentData()
        setClickCallbacks()
    }
    ...
    private fun getIntentData() {
        try {
            if (intent.hasExtra("videosList")) {
                var auList =
                    intent.getSerializableExtra("videosList") as ArrayList<ViewSavedMultimedia.ChatMultiMedia>
                videoList.addAll(auList)
                if (videoList.size==0)
                {
                    showToast("Videos not shared yet")
                }
            }
            setVideoListAdapter()
        } catch (e: Exception) {
                            ...
                            for (i in 0 until selectedMedia.size) {
                                if (names.contains(selectedMedia[i])) {
                                    ...
                                    if (i == selectedMedia.size - 1) {
                                        DownloadTask(
                                            this,
                                            WebUrls.CHAT_MEDIA_URL + videoList[indexIs].message,
                                            true
                                        )
                                    } else {
                                        DownloadTask(
    ...
    private fun setVideoListAdapter() {
        try {
            ...
            videoListAdapter!!.initOnClickChange(this)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    override fun checkChange(position: Int, isChecked: Boolean) {
        try {
            videoList[position].isSelected = isChecked
            if (isChecked) {
                if (!selectedMedia.contains(videoList[position].id)) {
                    selectedMedia.add(videoList[position].id)
                }
            } else {
                if (selectedMedia.contains(videoList[position].id)) {
                    var indexIs = selectedMedia.indexOf(videoList[position].id)
        ...
        try {
            startActivity(
                Intent(this, FullMediaScreen::class.java)
                    .putExtra("file_type", videoList[position].message_type)
                    .putExtra("message_type", "media")
                    .putExtra("message", videoList[position].message)
            )
        } catch (e: Exception) {
            e.printStackTrace()
        }
                ...
                try {
                    ...
                    if (jsonObject.getBoolean("response")) {
                        ...
                        for (i in 0 until selectedMedia.size) {
                            if (names.contains(selectedMedia[i])) {
                                var indexIs = names.indexOf(selectedMedia[i])
                                videoList.removeAt(indexIs)
                                videoListAdapter!!.notifyItemRemoved(indexIs)
                                videoListAdapter!!.notifyItemRangeChanged(indexIs, videoList.size)
                            }
                        }
                        selectedMedia.clear()
