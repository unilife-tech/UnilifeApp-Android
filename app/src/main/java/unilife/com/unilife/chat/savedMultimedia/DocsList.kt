...
import kotlinx.android.synthetic.main.docs_list_layout.*
...
class DocsList : AppDrawer(), DocsListAdapter.OnClickChange, View.OnClickListener,
    RetrofitResponse {
    private var selectedMedia = ArrayList<String>()
    private var alertDeleteChat: AlertDialog? = null

    private var docsList = ArrayList<ViewSavedMultimedia.ChatMultiMedia>()
    private var docsListAdapter: DocsListAdapter? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.docs_list_layout)
        ...
        tvMainTitle.text = getString(R.string.documents)
        selectedMedia.clear()
        getIntentData()
        setClickCallbacks()
    }
    ...
    private fun getIntentData() {
        try {
            if (intent.hasExtra("docsList")) {
                var auList =
                    intent.getSerializableExtra("docsList") as ArrayList<ViewSavedMultimedia.ChatMultiMedia>
                docsList.addAll(auList)
                if (docsList.size==0)
                {
                    showToast("Documents not shared yet")
                }
                            ...
                            for (i in 0 until selectedMedia.size) {
                                if (names.contains(selectedMedia[i])) {
                                    ...
                                    if (i == selectedMedia.size - 1) {
                                        DownloadTask(
                                            this,
                                            WebUrls.CHAT_MEDIA_URL + docsList[indexIs].message,
                                            true
                                        )
                                    } else {
                                        DownloadTask(
    ...
    private fun setdocsListAdapter() {
        try {
            docsListAdapter = DocsListAdapter(this, docsList)
            rvDocs.adapter = docsListAdapter
            docsListAdapter!!.initOnClickChange(this)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    override fun checkChange(position: Int, isChecked: Boolean) {
        try {
            docsList[position].isSelected = isChecked
            if (isChecked) {
                if (!selectedMedia.contains(docsList[position].id)) {
                    selectedMedia.add(docsList[position].id)
                }
            } else {
                if (selectedMedia.contains(docsList[position].id)) {
                    var indexIs = selectedMedia.indexOf(docsList[position].id)
        ...
        try {
            startActivity(
                Intent(this, FullMediaScreen::class.java)
                    .putExtra("file_type", docsList[position].message_type)
                    .putExtra("message_type", "media")
                    .putExtra("message", docsList[position].message)
            )
        } catch (e: Exception) {
            e.printStackTrace()
        }
                ...
                try {
                    ...
                    if (jsonObject.getBoolean("response")) {
                        ...
                        for (i in 0 until selectedMedia.size) {
                            if (names.contains(selectedMedia[i])) {
                                var indexIs = names.indexOf(selectedMedia[i])
                                docsList.removeAt(indexIs)
                                docsListAdapter!!.notifyItemRemoved(indexIs)
                                docsListAdapter!!.notifyItemRangeChanged(indexIs, docsList.size)
                            }
                        }
                        selectedMedia.clear()
