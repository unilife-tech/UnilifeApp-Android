...
public class EducationAdapter extends RecyclerView.Adapter<EducationAdapter.MyViewHolder> {
    ...
    ArrayList<UserEducation> arrayList=new ArrayList<>();;
    Context context;
    int which=0;

    // Provide a suitable constructor (depends on the kind of dataset)
    public EducationAdapter(Context context,int which) {
         this.context = context;
        this.which = which;
    }

    public void updateData(ArrayList<UserEducation> arrayList) {
    ...
    @Override
    public EducationAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent,
                                                            int viewType) {
        // create a new view
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        holder.txt1.setText(arrayList.get(position).getCollegeName());
        holder.txt2.setText(arrayList.get(position).getDegree());
        ...
        holder.txtDelete.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ((MyAccountActivity)context).deleteEducation(arrayList.get(position).getId());
            }
        });

        holder.imgLogo.setImageResource(R.drawable.image_education);
    }
